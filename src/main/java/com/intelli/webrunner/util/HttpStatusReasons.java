...
/** Standard HTTP status reason phrases used when Java HttpClient exposes only the status code. */
public final class HttpStatusReasons {
	...
	private HttpStatusReasons() {
	}

	public static String reason(int statusCode) {
		return REASONS.getOrDefault(statusCode, "");
	}

	public static String format(int statusCode, String statusMessage) {
		String message = statusMessage == null || statusMessage.isBlank()
			? reason(statusCode)
			: statusMessage.trim();
		return message.isEmpty() ? String.valueOf(statusCode) : statusCode + " - " + message;
	}
}
